...
def plan(
    ...
    from hermes_fleet.teams import load_team, load_role
    ...
    for agent_id in agents:
        role_data = load_role(agent_id) or {"name": agent_id.replace("-", " ").title()}
        if show_details:
            table.add_row(
                agent_id,
...
from hermes_fleet.agency_commands import agency_app
app.add_typer(agency_app, name="agency")


# ── Customize Subcommands ──────────────────────────────────────────
